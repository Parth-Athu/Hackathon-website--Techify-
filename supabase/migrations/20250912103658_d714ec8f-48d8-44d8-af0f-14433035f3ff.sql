-- Create updated user roles enum with admin
...
-- Create artwork categories enum
CREATE TYPE artwork_category AS ENUM ('gond', 'warli', 'bhil_pithora', 'madhubani', 'saura', 'pattachitra', 'other');

-- Create artwork status enum  
CREATE TYPE artwork_status AS ENUM ('draft', 'pending', 'approved', 'rejected');
...
CREATE TABLE IF NOT EXISTS public.artworks (
  ...
  cultural_info text, -- cultural significance and story
  tags text[],
...
CREATE POLICY "Everyone can view approved artworks" ON public.artworks
  FOR SELECT USING (status = 'approved');

CREATE POLICY "Sellers can view their own artworks" ON public.artworks
  FOR SELECT USING (
    seller_id IN (
...
CREATE POLICY "Sellers can create artworks" ON public.artworks
...
CREATE POLICY "Sellers can update their own artworks" ON public.artworks
  FOR UPDATE USING (
    seller_id IN (
      SELECT id FROM public.sellers WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Admins can view all artworks" ON public.artworks
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can update all artworks" ON public.artworks
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE INDEX IF NOT EXISTS idx_artworks_seller_id ON public.artworks(seller_id);
...
CREATE INDEX IF NOT EXISTS idx_artworks_region ON public.artworks(region);
